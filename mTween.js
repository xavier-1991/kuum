 ...
 * /*
				 ...
				 * 第一个参数：
				 * 		这个参数为变量
				 * 		要运动的对象，它是一个获取到元素。把它的变量放进去
				 * 第二个参数：
				 * 		这个参数为对象
				 * 		要写大括号
				 * 		要运动的属性，它是一个对象数据，要放在一对大括号中，大括号里放的是什么？放的是要运动的属性
				 * 
				 * 		对象数据：
				 * 			{"name":"老王"}
				 * 
				 * 		{"left":-730}	大括号中第一个为要运动的属性名，第二个为这个属性要运动到的目标值，没有单位，纯数字
				 * 
				 * 
				 * 第三个参数
				 * 		这个参数为数字
				 * 		时间，就是这个元素运动到目标值所用的时间，单位是毫秒
				 * 	500
				 * 
				 * 
				 * 第四个参数
				 * 		这个参数为字符串
				 * 		运动方式，它的方式是在mTween.js文件里面找，key的名字就是这个参数
				 * 
				 * 
				 * 第五个参数
				 * 		这个参数是个函数
				 * 		回调函数，当当前的属性运动到了目标值，就会自动执行这个回调函数
				 * 		
				 * 		这个参数可写可不写
				 * 		什么时候写：当你需要在一个属性运动完成后，去做别的事情，这个里写
				 * 
				 */
